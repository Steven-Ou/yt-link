...
def get_ffmpeg_path(base_path):
    """
    Determines the absolute path to the ffmpeg executable based on the OS.
    This is the core of the bug fix.
    """
    system = platform.system()
    if system == 'Windows':
        return os.path.join(base_path, 'ffmpeg.exe')
    # For macOS and Linux, the executable is just 'ffmpeg'
    return os.path.join(base_path, 'ffmpeg')


def get_playlist_index(filename):
    """Helper to sort playlist files numerically."""
    try:
        # Assumes format "1 - Title.mp3", "2 - Title.mp3", etc.
...
def post_download_processing(job_id, download_type):
    """Handles file operations after the download is complete (zipping, combining)."""
    job = jobs[job_id]
    temp_dir = job['temp_dir']
    playlist_title = job.get('playlist_title', 'download').replace('/', '_').replace('\\', '_') # Sanitize title
    ...
    if download_type == "singleMp3":
        ...
        with open(list_file_path, 'w', encoding='utf-8') as f:
            for file in mp3_files:
                # Use absolute paths and proper quoting for ffmpeg
        ...
        ffmpeg_exe = get_ffmpeg_path(job['ffmpeg_location'])
        ...
        # Using check=True will raise an exception if ffmpeg fails
...
def progress_hook(d):
    ...
    if job_id and jobs.get(job_id):
        if d['status'] == 'downloading':
            ...
            # Extract percentage and update job status
            percent_str = d.get('_percent_str', '0%').replace('%','').strip()
            jobs[job_id]['progress'] = percent_str
            jobs[job_id]['message'] = f"Downloading... {percent_str}%"
        elif d['status'] == 'finished':
            jobs[job_id]['status'] = 'processing'
            jobs[job_id]['message'] = 'Download finished, now converting to MP3...'
...
def start_job_endpoint():
    ...
    jobs[job_id] = {'status': 'queued', 'url': data['url'], 'ffmpeg_location': ffmpeg_location}

    # --- BUG FIX: Pass the full path to the executable, not just the directory ---
    ydl_opts = {
        'format': 'bestaudio/best',
        'postprocessors': [{'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '192'}],
        'ffmpeg_location': ffmpeg_location, # yt-dlp still expects the directory here
