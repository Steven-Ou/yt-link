...
def _process_combine_playlist_mp3_task(job_id, playlist_url, cookie_data):
    ...
    try:
        ...
        # --- KEY CHANGE FOR SAMPLE RATE STANDARDIZATION ---
        ffmpeg_pp_args = [
            "ffmpeg_o:-ar", "48000",
            "ffmpeg_o:-q:a", "3"     
        ]
        ytdlp_audio_args = [ 
            ...
            '-o', output_template, 
            '--no-warnings', '--verbose',
            '--postprocessor-args', ' '.join(ffmpeg_pp_args)
        ]
        # --- END OF KEY CHANGE ---
        ...
        logging.error(f"[{job_id}] {error_detail}", exc_info=False) 
        with jobs_lock:
            if job_id in jobs:
...
def start_single_mp3_job():
    request.job_id_in_context = "NEW_SINGLE_MP3" 
