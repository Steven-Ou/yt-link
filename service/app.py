...
# --- Basic Setup ---
# This ensures ffmpeg and ffprobe, which are required for audio conversion, are found.
...
def get_job_status_dict(job):
    ...
    return {
        'id': job.id, 'status': status, 'progress': progress,
        'message': message, 'downloadPath': download_path
    }

def progress_hook(d, job):
    """A hook for yt-dlp to report download progress back to the Redis job's metadata."""
...
def download_video(job_type, url, download_path, cookies_path=None):
    ...
    job.save_meta()
    ydl_opts = {
        'cookiefile': cookies_path, 'progress_hooks': [lambda d: progress_hook(d, job)],
    ...
    try:
        ...
        if job_type == 'playlist_zip':
            ...
            job.meta['download_path'] = os.path.dirname(zip_path)
        else:
...
def start_job_handler(job_type):
    ...
    try:
        job = q.enqueue(download_video, args=(job_type, url, download_path, cookies_path), job_timeout='2h')
        job.meta['download_path'] = download_path
        job.save_meta()
...
# These specific routes are required by the frontend's main.js file.
...
if __name__ == '__main__':
    ...
    # FIX: This print statement sends the "ready" signal to the Electron main process.
    # The `flush=True` is critical to ensure it's not buffered and gets sent immediately.
